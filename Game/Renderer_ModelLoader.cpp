...
void Renderer::loadModel(std::string modelPath, ModelN* n) {
    ...
    int32_t ipos = 0;
    for (int32_t ii = 0; ii < eng.getTrianglesDatas().size(); ii++) {
        TrianglesData* dt = &eng.getTrianglesDatasP()->at(ii);
        if (dt->modelPath == modelPath) {
            dt->usedCount++;
            modelFound = true;
            n->trianglesData = ipos;
            break;
        }
        ipos++;
    }
    ...
    if (modelFound == false) {
        ...
        for (const auto& shape : shapes) {
            for (const auto& index : shape.mesh.indices) {
                ...
                dt.indices.push_back(uniqueVertices[vertex]);
                //indices.push_back(indices.size());
            }
        }
        dt.indicesSize = dt.indices.size();

        n->trianglesData = ipos;
        eng.getTrianglesDatasP()->push_back(dt);
    }
}
